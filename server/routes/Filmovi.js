...
router.get("/", async (req, res) => {
  ...
  await Filmovi.findAll().then((filmovi) => {
    const repertoar = filmovi.map((film) => {
      return {
        id: film.id,
        slika: film.slika,
        orginalniNaslov: film.orginalniNaslov,
        termini: film.termini,
        zanr: film.zanr,
      };
    });
...
router.get("/:id", async (req, res) => {
  await Filmovi.findByPk(req.params.id).then((film) => {
    return res.json(film);
  });
});

router.get("/uskoro/:id", async (req, res) => {
  await Uskoro.findByPk(req.params.id).then((film) => {
    return res.json(film);
  });
});
